...
void TransformationItem::apply(QGraphicsItem* item) {
    ...
    switch (m_operation) {
        ...
        case SHEAR: item->setTransform(QTransform().shear(m_x, m_y), true);
            break;
        case SCALE: item->setTransform(QTransform().scale(m_x, m_y), true);
            break;
        case TRANSLATE: item->setTransform(QTransform().translate(m_x, m_y), true);
            break;
