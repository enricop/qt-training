...
if(ret == QMessageBox::Ok) {
  ...
  \item Very flexible in appearance
    \begin{itemize}
    \item \doc{qmessagebox.html}{QMessageBox Class Reference}
    \end{itemize}
...
dialog.setValue(count); // ensure set to maximum      
  ...
  \item When operation progresses, check for cancel
    \begin{itemize}
    \item \iClsFn{QProgressDialog}{wasCanceled}
    \item Or connect to \iClsFn{QProgressDialog}{canceled}
    \end{itemize}
  \item To stay reactive call \iClsFn{QApplication}{processEvents}
  \item For modeless operation: \doc{qprogressdialog.html}{QProgressDialog}
  \end{itemize}
\end{slide}
