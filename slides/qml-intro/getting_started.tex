...
Behind the hood, Qt Quick uses:
\begin{itemize}
\item 2 threads: GUI and Render (OpenGL) thread
\item an OpenGL scene graph, designed specifically for QML
\item synchronisation between the two threads (for animation) driven by the vertical blank signal
\end{itemize}

\centeredImage{qml-intro/images/render-threading.png}
